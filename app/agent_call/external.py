...
def format_github_request(payload):
    ...
    if r.status_code == 200:
        ...
        changed_files = [commit_update(commit) for commit in commits if (commit['filename'].find('/lib/') < 0 and  commit['filename'].find('/bin/') < 0 and  commit['filename'].find('ipynb_checkpoints/') < 0)]
